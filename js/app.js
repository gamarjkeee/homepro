(() => {
    ...
    function buildBlogItem(item) {
        ...
        blogItemTemplate += `<div data-id="${item.id}" class="blog__item item-blog">`;
        if (item.image) blogItemTemplate += `<a href="${item.url}" class="item-blog__image">\n         <img src="${item.image}" alt="Image" />\n    </a>`;
        blogItemTemplate += `<div class="item-blog__content">`;
        blogItemTemplate += `<div class="item-blog__date">${item.date}</div>`;
        blogItemTemplate += `  <h4 class="item-blog__title">\n  <a href="${item.url}">${item.title}</a>\n</h4>`;
        if (item.text) blogItemTemplate += `<div class="item-blog__text">\n${item.text}\n</div>`;
        if (item.labels) {
            blogItemTemplate += `<div class="item-blog__labels">`;
            for (const label in item.labels) blogItemTemplate += `<a href="${item.labels[label]}" class="item-blog__label">\n      ${label}\n    </a>`;
    ...
    function viewMore() {
        ...
        const viewMoreButton = document.querySelector(".blog__button");
        if (currentItemsLength < dataItemsLength) ; else viewMoreButton.classList.add("hidden");
    }
    ...
    let spollerItems = document.querySelectorAll(".spollers__item");
    console.log(spollerItems);
    spollerItems.forEach((item => {
        if (item.open) item.classList.add(".open");
    }));
    window["FLS"] = false;
    isWebp();
    addLoadedClass();
